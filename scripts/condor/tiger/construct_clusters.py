...
def analyse():
  ...
  root_command = ['root', '-b', '-q', '-e', f'gROOT->ProcessLine(".L link.C"); gROOT->ProcessLine(".L tiger.C"); gROOT->ProcessLine("(new tiger(\\"{filename}\\", \\"{directory}\\", \\"{mapping}\\", \\"20221025\\"))->FindClusters()")']
  print(root_command)
  subprocess.run(root_command);

  shutil.copy(f'../out/tiger_clusters_{directory}-{filename}.root', outdir)
